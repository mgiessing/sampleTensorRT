...
class VisionCODModelDeploy(object):
    ...
    def __init__(self, proto, model, json_file, labels, batch_size, resolution):

        attribute_file = " "

        self._parseModelInfo(json_file, labels, attribute_file)
        ...
        self.trt_inference_wrapper = inference_utils.TRTInference(
            proto, trt_engine_path, model,
            trt_engine_datatype=trt_engine_datatype,
            batch_size=batch_size)

    def inference(self, image, conf_threshold, showgui=False):
        objects = self._detect(image, conf_threshold)
        return objects

    def _parseModelInfo(self, json_file, labels, attribute_file):
    ...
    def _detect(self, image, conf_threshold, showgui=False):
...
def main(argv):
    ...
    detector = VisionCODModelDeploy(args.proto, args.model, args.json_file, args.labels, args.bs, args.resolution)
    if args.image != 'Camera':
        results = detector.inference(args.image, args.confthre, args.showgui)
        print(results)
    else:
        ...
        if cap.isOpened():
            while True:
                _, img = cap.read()
                results = detector.inference(img, args.confthre, args.showgui)
                print(results)
