...
class Detector():
    def __init__(self, proto, model, labels, gpu_mode, trt_mode, batch_size, resolution):
        ...
        ssd_model_path = model
        ssd_deploy_path = proto
    ...
    def detect(self, image, conf_thresh=0.5, showgui=False):
        '''
        SSD detection
        '''
...
def main(argv):
    ...
    detector = Detector(args.proto, args.model, args.labels, True, True, args.bs, args.resolution)
    if args.image != 'Camera':
        results = detector.detect(args.image, args.confthre, args.showgui)
        print(results)
    else:
        ...
        if cap.isOpened():
            while True:
                _, img = cap.read()
                results = detector.detect(img, args.confthre, args.showgui)
                print(results)
                key = cv2.waitKey(1)
